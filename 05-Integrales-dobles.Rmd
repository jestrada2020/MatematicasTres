...
```{example name="Volumen de un solido con región rectángular"}
...
Esta es una aplicación que generar un sólido y proporciona su volumen, inicialmente la realizó en youtube Matemática y Geogebra (https://www.youtube.com/watch?v=GfhDeC2lN88&t=857s), y posteriormente la realizó John Jairo Estrada (https://www.geogebra.org/classic/f6guccy3).
...
```{exercise name="Volumen de un solido con región rectángular"}
...
```{solution name="Volumen de un solido con región rectángular"}
$$
...
```{theorem name="Región verticalmente simple"}
...
```{theorem name="Región horizontalmente simple"}
Supongamos que $f(x,y)$ es continuas en una región cerrada y acotada $D$ del plano $XY$. Si $D$ es una región verticalmente simple, entonces $D$ se puede describir en términos de desigualdades de la forma:
...
```{exercise name="Volumen de un solido con región no rectángular"}
...
```{exercise name="Área entre dos curvas"}
...
```{solution name="Área de una región que requiere de dos integrales dobles"}
